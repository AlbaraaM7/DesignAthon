...
const DriftWall = ({
  ...
  useLayoutEffect(() => {
    ...
    const ro = new ResizeObserver(([entry]) => {
      setContainerHeight(entry.contentRect.height || 550);
    });
  ...
  const renderTile = (item, id, colIndex) => {
    return (
      <div
        ...
        onBlur={release}
      >
        <div className="drift-wall__inner group">
          <img src={item.image} alt={item.title ?? ''} loading="lazy" decoding="async" draggable={false} />
          <div className="drift-wall__content absolute inset-0 p-3 flex flex-col justify-between bg-gradient-to-t from-black/90 via-black/40 to-transparent">
            ...
            <div>
              ...
              <p className="text-[10px] text-gray-400 truncate">{item.university || 'Verified Student'}</p>
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div
      ...
      role="group"
      aria-label="Drifting student showcase"
    >
